...
for i in range(len(img_information)):
    if img_information[i][1].strip() != '_missing descriptor)':
        ...
        expression = img_information[i][4].strip()
        if sex == '_sex  male)' and (expression == '_face smiling)' or expression == '_face funny)'):
            imginfor.append('positive_male')
        elif sex == '_sex  male)' and expression == '_face serious)':
            imginfor.append('negative_male')
        elif sex == '_sex  female)' and (expression == '_face smiling)' or expression == '_face funny)'):
            imginfor.append('positive_female')
        else:
            imginfor.append('negative_female')
    else:
        imginfor.append('missing')
        wrong.append(i)
# 删除缺失图像信息的信息
index_offset = 0          #索引的偏移量：因为下面是逐次删除，所以每当前一次删除后，后面的索引要相应减一
...
print("Total face:%d\nPositive_male:%d\nNegative_male:%d\nPositive_female:%d\nNegative_female:%d\n"
      % (len(imginfor),imginfor.count('positive_male'),imginfor.count('negative_male'),
       imginfor.count('positive_female'),imginfor.count('negative_female')))  #类别样本数 1878 2013 100
print("=======================================================")
f.close()
...
#### PCA+LDA
PCA_LDA = LinearDiscriminantAnalysis(n_components = 3).fit(X_train_PCA,y_train_PCA)
...
for L,C,M in zip(np.unique(y_train_PCA),colors,markers):
    for i in range(0,2793):
        if y_train_PCA[i] == L:
            ...
            continue
plt.title('Train Data In Gender-Expression Sample Set')
plt.xlabel('LD 1')
plt.ylabel('LD 2')
...
for L,C,M in zip(np.unique(y_train_PCA),colors,markers):
    for i in range(0,1198):
        if y_test_PCA[i] == L:
            ...
            continue
plt.title('Test Data In Gender-Expression Sample Set')
plt.xlabel('LD 1')
plt.ylabel('LD 2')
# plt.legend(loc='lower right')
plt.show()
...
for L,C,M in zip(np.unique(y_train_PCA),colors,markers):
    for i in range(0,1198):
        if y_pred_bayes_PL[i] == L:
            ...
            continue
plt.title('Predict Data In Gender-Expression Sample Set')
plt.xlabel('LD 1')
plt.ylabel('LD 2')
# plt.legend(loc='lower right')
plt.show()
...
plt.ylabel('Accuracy')
plt.title('The accuracy of different PCA dimension of Gender-expression classification')